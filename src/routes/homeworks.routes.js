...
const storage = multer.diskStorage({
  destination: async (req, file, cb) => {
    try {
      ...
      if (!fs.existsSync(path.join(__dirname, `/../uploads/${nameFolderTarea}/${nuevoId}`))) {
        fs.mkdirSync(path.join(__dirname, `/../uploads/${nameFolderTarea}/${nuevoId}`))
      }
      req.nameFolderTarea = nameFolderTarea;
      req.id_class = selecClass[0].id_class;

      //guardado de archivo
      cb(null, `src/uploads/${nameFolderTarea}/${nuevoId}`)
    } catch (error) {
      console.log(error);
    }

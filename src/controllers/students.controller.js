...
exports.getPendings = async (req, res) => {
  try {
    const id_student = await query(`SELECT id_student FROM students WHERE id_user = ${req.userFound[0].id_user}`)
    const pendings = await query(`SELECT * FROM pendings JOIN homeworks ON homeworks.id_homework = pendings.id_homework WHERE id_student = ${id_student[0].id_student} AND status = "ACTIVE"`);
    res.status(200).json({ ok: true, message: "Estas son todos los pendientes del alumno", pendings })
  } catch (error) {
    res.status(500).json({ ok: false, message: "Algo ha fallado", error })
  }
}
...
exports.getAllHomeworks = async (req, res) => {
  try {
    const id_student = await query(`SELECT id_student FROM students WHERE id_user = ${req.userFound[0].id_user}`)
    const homeworks = await query(`SELECT * FROM pendings WHERE id_student = ${id_student[0].id_student}`);
    res.status(200).json({ ok: true, message: "Estas son todos los pendientes del alumno", homeworks })
  } catch (error) {
    res
  }
}

getClassesStudent = async (req, res) => {
